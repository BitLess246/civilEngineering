...
Paths below are relative to this skill directory (`.claude/skills/svg-precision/`).

**Interpreter:** use `python` in a local Windows session and `python3` in a Linux
cloud session. Do not probe with `command -v python3` on Windows — it resolves to
the Microsoft Store stub, which is on PATH but is not a working Python.
