...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.getElementById('formFoundation').addEventListener('submit', function(event) {
        ...
        try {
            ...
            if (solution === 11) {
                ...
                /*punchingshear = punchingShear(dc, depth, soilBearingCapacity,unitWeightConcrete,unitWeightSoil,rectangularDimension,liveLoad,deadLoad,clearCover,barDia,punchingShearVu,surcharge,columnWidth,λ,fc,ddd,dc1,dc2,rectangularDimension1,punchingShearVu1)
                console.log("22-9.5"); 
                beamshear = beamShear(dc2,clearCover,barDia,beamShearResult,rectangularDimension1,columnWidth,punchingShearVu1,λ,fc)
                console.log("22-11"); */
                ...
                // Display results
               /* const results = compileDisplay(punchingshear, beamshear);
                console.log("22-10"); 
                resContent = results.innerText;*/
    ...
    /**
     * Calculates the dimension square
     */
    ...
    /**
     * Calculates the punching shear force
     */
   ...
   /**
 * Calculates the punching shear Vn values.
 * @param {number} d Effective depth of the concrete.
 * @param {number} side The side length of the critical section.
 * @param {number} fc The compressive strength of the concrete.
 * @param {number} λ Factor for lightweight concrete.
 * @param {number} bRatio The aspect ratio of the loaded area.
 * @param {number} as The column location factor.
 * @returns {Object} An object containing the Vn1, Vn2, and Vn3 values.
 */
function calculatePunchingShearVn(d, side, fc, λ, bRatio, as) {
    ...
    /**
 * Determines the 'as' value based on column location
 * @param {number} columnLocation The location of the column (typically an integer value)
 * @returns {number} The corresponding 'as' value for the calculation
 */
function columnLoc(columnLocation) {
    ...
    /**
     * Determines the solution method based on structure type and method
     */
    ...
    function compileDisplay(beamshear, punchingshear, finalDC, finalB, finalL) {
        // Create a container for the entire results
        const resultsContent = document.createElement('div');
    
        // Populate the header and summary sections
        resultsContent.innerHTML = `
            <h3>Solution:</h3>
            <h5>Punching Shear Calculation</h5>
        `;
    
        // Convert beamshear and punchingshear to nodes if they are not already
        if (typeof punchingshear === 'string') {
            const punchingshearNode = document.createElement('div');
            punchingshearNode.innerHTML = punchingshear;
            punchingshear = punchingshearNode;
        }
    
        if (typeof beamshear === 'string') {
            const beamshearNode = document.createElement('div');
            beamshearNode.innerHTML = beamshear;
            beamshear = beamshearNode;
        }
    
        // Append the punching shear and beam shear calculations directly
        resultsContent.appendChild(punchingshear);
        resultsContent.innerHTML += `<h5>Beam Shear Calculation</h5>`;
        resultsContent.appendChild(beamshear);
        ...
        // Append the newly created results content to the designated container
    ...
    function punchingShear(dc, depth, soilBearingCapacity,unitWeightConcrete,unitWeightSoil,Dimension,liveLoad,deadLoad,clearCover,barDia,punchingShearVu,surcharge,columnWidth,λ,fc,dkk,dc1,dc2,Dimension1,punchingShearVu1){
        console.log("22-9.1"); 
        ...
        resultsContent.innerHTML = `
              ...
              <p>q<sub>net</sub> = q<sub>a</sub> - γ<sub>c</sub> D<sub>c</sub> - γ<sub>s</sub> D<sub>s</sub> - q </p>
              <p>q<sub>net</sub> = ${soilBearingCapacity} kPa - (${unitWeightConcrete} kN/m<sup>3</sup> x ${dc/1000} m) - (${unitWeightSoil} kN/m<sup>3</sup> x ${Dimension.ds/1000} m) - ${surcharge} kN/m<sup>2</sup></p>
              <p>q<sub>net</sub> = ${Dimension.qnet} kPa</p>
              <p>P = Live Load + Dead Load = ${liveLoad} kN + ${deadLoad} kN =${liveLoad+deadLoad} kN</p>
              <p>A<sub>f</sub> = P / q<sub>net</sub> = ${Dimension.P} kN / ${Dimension.qnet} kPa = ${Dimension.Af.toFixed(2)} m<sup>2</sup></p>
              <p>BL = A<sub>f</sub> = ${Dimension.Af.toFixed(2)} m<sup>2</sup> / ${Dimension.limitLength.toFixed(2)} meters</p>
              <p>L = ${Dimension.B3}</p>
              <p>L ≈ ${Dimension.L} meters , B = ${Dimension.B4}</p>
              <p>${Dimension.L} meters ${Dimension.L>Dimension.limitLength?'>':'<'} ${Dimension.limitLength} meters</p>
              <p>Therefore, ${Dimension.B2>Dimension.limitLength?'Rectangular Footing':'Square Footing'}</p> 
              <p>d (effective depth) = D<sub>c</sub> - C<sub>c</sub> -d<sub>b</sub> = ${dc} mm - ${clearCover} mm - ${barDia} mm = ${punchingShearVu.d} mm</p>
              <p>P<sub>u</sub> (Ultimate Load) = 1.2 Dead Load + 1.6 Live Load + 1.2 [(γ<sub>s</sub> x d<sub>s</sub>) + (γ<sub>c</sub> x d<sub>c</sub>) + q ] x B x L </p>
              <p>P<sub>u</sub> (Ultimate Load) = 1.2(${deadLoad} kN) + 1.6(${liveLoad} kN) + 1.2 [( ${unitWeightSoil} x ${(depth-dc)/1000} ) + ( ${unitWeightConcrete} x ${dc/1000} ) + ${surcharge} ] x ${Dimension.B4} x ${Dimension.L} = ${punchingShearVu.Pu} kN</p>
              <p>q<sub>u</sub> (Ultimate Bearing Pressure): P<sub>u</sub> / A<sub>f</sub> = ${punchingShearVu.Pu} kN / (${Dimension.B4} meters x ${Dimension.L} meters) = ${punchingShearVu.qu.toFixed(2)} kPa</p>
              ...
              <p>ΦV<sub>n</sub> (Punching Shear) = 0.75 x 0.33 x λ x √f'c x 4 x (C + d) x d </p>
              ...
              <p>q<sub>net</sub> = q<sub>a</sub> - γ<sub>c</sub> D<sub>c</sub> - γ<sub>s</sub> D<sub>s</sub> - q </p>
              <p>q<sub>net</sub> = ${soilBearingCapacity} kPa - (${unitWeightConcrete} kN/m<sup>3</sup> x ${dc2/1000} m) - (${unitWeightSoil} kN/m<sup>3</sup> x ${Dimension1.ds/1000} m) - ${surcharge} kN/m<sup>2</sup></p>
              <p>q<sub>net</sub> = ${Dimension1.qnet} kPa</p>
              <p>A<sub>f</sub> = P / q<sub>net</sub> = ${Dimension1.P} kN / ${Dimension1.qnet} kPa = ${Dimension1.Af.toFixed(2)} m<sup>2</sup></p>
              <p>L = A<sub>f</sub>/B = ${Dimension1.Af.toFixed(3)} / ${Dimension1.limitLength.toFixed(3)} meters = ${Dimension1.B3}</p>
              <p>L ≈ ${Dimension1.L} meters</p>
              ...
              <p>q<sub>u</sub> (Ultimate Bearing Pressure): P<sub>u</sub> / A<sub>f</sub> = ${punchingShearVu1.Pu} kN / (${Dimension1.B4} meters x ${Dimension1.L} meters) = ${punchingShearVu1.qu.toFixed(2)} kPa</p>
    ...
    function beamShear(dc2,clearCover,barDia,beamShearResult,Dimension1,columnWidth,punchingShearVu1,λ,fc ){
        ...
        // Populate the results with formatted content
        resultsContent.innerHTML = `
        <p>Shear Along Long Span:</p>
        <p>d = D<sub>c</sub> - C<sub>c</sub> - 0.5d<sub>b</sub> = ${dc2} mm - ${clearCover} mm - 0.5(${barDia} mm) = ${beamShearResult.xAxis.d} mm</p>
        <p>aa = (L - c - 2d) / 2 = (${Dimension1.L * 1000} mm - ${columnWidth} mm - 2(${beamShearResult.xAxis.d} mm)) / 2 = ${beamShearResult.xAxis.aa.toFixed(2)} mm</p>
        <p>V<sub>u</sub> = q<sub>u</sub> * B * aa = ${punchingShearVu1.qu.toFixed(2)} kPa * ${Dimension1.B4} m * ${beamShearResult.xAxis.aa / 1000} m = ${(beamShearResult.xAxis.Vu/1000000).toFixed(2)} kN</p>
        <p>ΦV<sub>n</sub> = (1/6) * λ * √f'c * B * d = (1/6) * ${λ} * √${fc} * ${Dimension1.B4 * 1000} mm * ${beamShearResult.xAxis.d} mm = ${(beamShearResult.xAxis.ΦVn/1000).toFixed(2)} kN</p>
        <p>Status: ${(beamShearResult.xAxis.Vu/1000000) > (beamShearResult.xAxis.ΦVn/1000) ? 'Failed' : 'Passed'}</p>
        <p>Shear Along Short Span:</p>
        <p>d = D<sub>c</sub> - C<sub>c</sub> - 1.5d<sub>b</sub> = ${dc2} mm - ${clearCover} mm - 1.5(${barDia} mm) = ${beamShearResult.yAxis.d} mm</p>
        <p>aa = (B - c - 2d) / 2 = (${Dimension1.B4 * 1000} mm - ${columnWidth} mm - 2(${beamShearResult.yAxis.d} mm)) / 2 = ${beamShearResult.yAxis.aa.toFixed(2)} mm</p>
        <p>V<sub>u</sub> = q<sub>u</sub> * L * aa = ${punchingShearVu1.qu.toFixed(2)} kPa * ${Dimension1.L} m * ${beamShearResult.yAxis.aa / 1000} m = ${(beamShearResult.yAxis.Vu/1000000).toFixed(2)} kN</p>
        <p>ΦV<sub>n</sub> = (1/6) * λ * √f'c * L * d = (1/6) * ${λ} * √${fc} * ${Dimension1.L * 1000} mm * ${beamShearResult.yAxis.d} mm = ${(beamShearResult.yAxis.ΦVn/1000).toFixed(2)} kN</p>
...
function rebarDisplay(B,L,Dc,Cc,db,diaBar, c, qu, fc, fy, b, bp, d, aa, Mu, ct, beta, num, at, reductionFactor, muMax, SRRB, Rn, rho, rhoMin1, rhoMin2, rhoMin,as1, as, asMin, asMin1, asMin2, ab, nInitial, nRounded, sc, scMin, message, centerBand, nCenterBand, nCenterBandRounded, Ag,beta1){
    ...
    */
    ...
    Check if SRRB
        ...
        } else if (0.65>=(0.85 - ((0.05/7)*(fc-28)))) {
            let beta1 = 0.65;

        } else {
            let beta1 = 0.85 - ((0.05/7)*(fc-28));
        }
        let at = beta1 * ct;
        let reductionFactor = 0.9;
        let muMax = reductionFactor * (0.85 * fc) * at * b * (d-(at/2));
        // compare Mu with muMax
        if (Mu < muMax) {
            let SRRB = true;
        } else {
    ...
    */
    ...
    if (SRRB == true) {
            ...
            nInitial = as / ab;
            nRounded = Math.ceil(nInitial);
            sc = (b-(2*Cc)-(nRounded*diaBar))/(nRounded-1);
            scMin = Math.max(50, diaBar);
            console.log("d12");
            if (sc < scMin) {
                message = "Spacing Fails, Increase Dimension if Possible";
            } else {
                message = "Okay";
            }
            centerBand = 2 / (beta1+1);
            nCenterBand = centerBand * nRounded;
            nCenterBandRounded = Math.ceil(nCenterBand);
        } else {
            console.log("DRRB");
