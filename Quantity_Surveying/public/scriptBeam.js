  ...
  document.getElementById('formBeam').addEventListener('submit', function(event) {
    ...
    try {
      ...
      if (diameterBottomMidspan === diameterBottomSupport || diameterBottomMidspan === diameterTopMidspan || diameterBottomMidspan === diameterTopSupport){ 
        ...
        let mainSteel = calculateSteelWeight(totalNetLength, diameter, spliceLength);
        textSteel = `
        ...
        textSteel2 = `
        <p>No. of Bars: ${mainSteel.noOfPcs} pieces</p>
        ...
        console.log("8");
        let mainSteelTopSup = calculateSteelWeight(topSupNetLength.netLength, diameterTopSupport, spliceLength);
        let mainSteelTopMid = calculateSteelWeight(topMidNetLength.netLength, diameterTopMidspan, spliceLength);
        let mainSteelBotSup = calculateSteelWeight(botSupNetLength.netLength, diameterBottomSupport, spliceLength);
        let mainSteelBotMid = calculateSteelWeight(botMidNetLength.netLength, diameterBottomMidspan, spliceLength);
        console.log("9");
        textSteel = `
        <p>Net Length (Top Bars @ Support) = ${topSupNetLength.lengthPerPiece} * ${topSupNetLength.numBars} *${topSupNetLength.numStructures} = ${topSupNetLength.netLength} meters </p>
        <p>Net Length (Top Bars @ Midspan) = ${topMidNetLength.lengthPerPiece} * ${topMidNetLength.numBars} *${topMidNetLength.numStructures} = ${topMidNetLength.netLength} meters </p>
        ...
        <p>No. of Bars (Top Bars @ Support) = ${mainSteelTopSup.netLength} / ${mainSteelTopSup.splice} ≈ ${mainSteelTopSup.noOfPcs} pieces</p>
        <p>No. of Bars (Top Bars @ Midspan) = ${mainSteelTopMid.netLength} / ${mainSteelTopMid.splice} ≈ ${mainSteelTopMid.noOfPcs} pieces</p>
        <p>No. of Bars (Bottom Bars @ Support) = ${mainSteelBotSup.netLength} / ${mainSteelBotSup.splice} ≈ ${mainSteelBotSup.noOfPcs} pieces</p>
        <p>No. of Bars (Bottom Bars @ Midspan) = ${mainSteelBotMid.netLength} / ${mainSteelBotMid.splice} ≈ ${mainSteelBotMid.noOfPcs} pieces</p>
...
function calculateSteelWeight(netLength, dia, spliceLength) {
  ...
  return {steelWeight, area, netLength, dia, noOfPcs, splice };
}
...
function displayResults(volumeConc, materials, reinforcementSteel, tieWire, text1, text2) {
  ...
  resultsContent1.innerHTML = `
    ...
    ${text2}
    <p><h5>Stirrups:</h5></p>
    <p>No. of Bars: ${reinforcementSteel.noOfPcs} pieces</p>
