...
import { PageHeader, DrawingCard } from '../components/calc'
import { DrawingFrame } from '../components/DrawingFrame'
import { ReportControls } from '../components/ReportControls'
import { WorkedSolution } from '../components/WorkedSolution'
import type { SolutionStep } from '../lib/solution'

// Influence Lines — bridge truss. A unit load walks the deck one panel point
// at a time; the engine solves the whole determinate truss at each position
// (engine/influenceTruss.ts) and every member gets its polygonal influence
// line. The page scrubs the load continuously (between panel points the deck
// beams share it, so the ordinate interpolates linearly), highlights members
// by their current force, and integrates a uniform load patch over [a, b].
...
const MUTED = '#5c6675'
const HAIR = '#e3e1da'
const TINT_T = '#eaf1f9'   // tension fill
const TINT_C = '#fbeaea'   // compression fill

const f2 = (v: number) => (Number.isFinite(v) ? v.toFixed(2) : '—')
const f3 = (v: number) => (Number.isFinite(v) ? v.toFixed(3) : '—')
const signed = (v: number) => `${v >= 0 ? '+' : '−'}${f2(Math.abs(v))}`
...
export default function InfluenceLines() {
  ...
  if (!res) {
    return (
      <div>
        <PageHeader title="Influence Lines — Bridge Truss" badges={['Simply supported', 'Unit-load method']} />
