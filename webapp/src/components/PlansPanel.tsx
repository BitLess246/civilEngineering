...
function Sheet({ sheet, svg, onOpen }: { sheet: PlanSheet; svg: string; onOpen: () => void }): JSX.Element {
  return (
    <div className="space-y-1">
      <div className="rounded-xl border border-hairline bg-sheet">
        <div className="flex items-center justify-between border-b border-hairline-2 px-3 py-1.5">
        ...
        <RawSvg svg={svg} onClick={onOpen} role="button" tabIndex={0}
          ...
          className="cursor-zoom-in select-none overflow-x-auto p-3 hover:bg-sheet-2/70" />
      </div>
    </div>
  )
}
