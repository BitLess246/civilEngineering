...
/** CPM day values are whole numbers (durations and lags are integer days);
 *  show them without decimals, and one decimal only if an edit ever makes
 *  one fractional — never a raw 9.500000000000002 spilling out of a cell. */
const fmtDay = (v: number) => (Number.isInteger(v) ? String(v) : f1(v))
  ...
  return (
    <div className="mt-6 space-y-4 break-before-page">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        {[
          ['Project duration', `${fmtDay(solved.projectDays)} days`],
          ['Uncertainty (PERT σ)', `${f1(solved.projectSd)} days`],
      ...
      <div className="overflow-x-auto rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <p className="mt-2 text-[11px] text-slate-500">
          ...
          matches its network diagram; PERT adds the uncertainty spread (σ {f1(solved.projectSd)} d, P80
          {' '}{f1(solved.projectDays + 0.842 * solved.projectSd)} d). Verify crew sizes, procurement and a working calendar for a contractual programme.
        </p>
      </div>
    </div>
  )
