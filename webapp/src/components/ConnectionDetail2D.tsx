...
export function ConnectionDetail2D({ conn, hostShape, hostKind, faceType, beamShape }: {
  ...
  return (
    <div className="flex w-full flex-wrap items-start gap-4 [&>div]:min-w-[260px] [&>div]:flex-1">
      {/* ELEVATION */}
      <DrawingFrame label="connection elevation">
          <svg viewBox={`0 0 ${W} ${H}`} className="h-auto w-full rounded-lg border border-hairline bg-sheet" style={{ maxWidth: W * sc }}>
      ...
      {/* END SECTION */}
      <DrawingFrame label="connection end section">
          <svg viewBox={`0 0 ${W2} ${H2}`} className="h-auto w-full rounded-lg border border-hairline bg-sheet" style={{ maxWidth: W2 * sc }}>
