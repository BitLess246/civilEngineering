...
export function AccountMenu({ dark }: { dark?: boolean }) {
  ...
  if (loading) {
    return <span className={dark ? 'px-2 text-[12.5px] text-rail-muted' : 'px-2 text-[12px] text-faint'}>…</span>
  }
    ...
    <div className="relative" ref={box}>
      <button type="button" onClick={() => setOpen((v) => !v)}
        ...
        className={`flex items-center gap-2 rounded-md px-1.5 py-1 ${
          dark ? 'hover:bg-sheet/10' : 'hover:bg-paper'}`}>
        <span className="flex h-6 w-6 flex-none items-center justify-center rounded-full bg-brand text-[11px] font-bold text-on-solid">
          {initial}
        </span>
        <span className={`hidden max-w-[140px] truncate text-[12px] font-semibold sm:inline ${
          dark ? 'text-white' : 'text-ink'}`}>{label}</span>
        <span className={`hidden rounded px-1.5 py-px font-mono text-[9.5px] font-semibold uppercase sm:inline ${
          dark ? 'bg-sheet/15 text-rail-ink' : 'bg-brand-tint text-brand'}`}>{plan.name}</span>
        <svg viewBox="0 0 24 24" width="11" height="11" fill="none" stroke="currentColor" strokeWidth="3"
          className={dark ? 'text-rail-muted' : 'text-faint'}><polyline points="6 9 12 15 18 9" /></svg>
      </button>
