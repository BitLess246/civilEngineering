...
/**
 * The pieces of a cage's steel lying WITHIN a slice of the cutting plane,
 * projected onto it.
 *
 * A plan section is a SLICE, not a plane. A beam's top bar lies in the cut for
 * six metres and then turns down out of it at the hook, so `cutCage` — which
 * asks whether a run lies in the plane AS A WHOLE — finds it neither flat nor
 * crossing, and the bars that enter the joint are exactly the ones it drops.
 * Clipping each run to |s| ≤ half the thickness instead, the straight length
 * shows and the hook simply stops where it leaves the slice, which is what a
 * plan section of a hooked bar looks like.
 *
 * `thickness` is the full depth of the slice, m.
 */
export function sliceCage(
  cage: RebarCage, cut: CageCut, thickness: number,
  keep: (run: RebarRun) => boolean = () => true,
): CutTie[] {
  const n = unit(cut.normal)
  const h = Math.max(thickness, 0) / 2
  const s = (p: Vec3) => dot3(sub3(p, cut.at), n)
  const out: CutTie[] = []
  for (const run of cage.runs) {
    if (!keep(run)) continue
    for (const poly of runPolylines(run)) {
      let cur: Vec3[] = []
      const flush = () => {
        if (cur.length > 1) {
          out.push({
            pts: cur.map((p) => projectPoint(p, cut.plane)),
            dia: run.dia, role: run.role, mark: run.mark,
            offset: Math.min(...cur.map((p) => Math.abs(s(p)))),
          })
        }
        cur = []
      }
      const at = (a: Vec3, b: Vec3, target: number): Vec3 => {
        const sa = s(a), sb = s(b)
        const t = Math.abs(sb - sa) < 1e-12 ? 0 : (target - sa) / (sb - sa)
        return [a[0] + (b[0] - a[0]) * t, a[1] + (b[1] - a[1]) * t, a[2] + (b[2] - a[2]) * t]
      }
      const inside = (p: Vec3) => Math.abs(s(p)) <= h + 1e-9
      for (let k = 0; k < poly.length; k++) {
        const p = poly[k]!
        if (k === 0) { if (inside(p)) cur.push(p); continue }
        const a = poly[k - 1]!, b = p
        const ai = inside(a), bi = inside(b)
        if (ai && bi) { cur.push(b); continue }
        const edge = (from: Vec3, to: Vec3) => at(from, to, s(to) < -h ? -h : h)
        if (ai && !bi) { cur.push(edge(a, b)); flush(); continue }
        if (!ai && bi) { cur = [edge(b, a), b]; continue }
        // both outside, on opposite sides — it crosses the whole slice
        if ((s(a) < -h && s(b) > h) || (s(a) > h && s(b) < -h)) {
          out.push({
            pts: [at(a, b, s(a) < 0 ? -h : h), at(a, b, s(a) < 0 ? h : -h)]
              .map((q) => projectPoint(q, cut.plane)),
            dia: run.dia, role: run.role, mark: run.mark, offset: 0,
          })
        }
      }
      flush()
...
/** `sliceCage` over several cages — every beam that frames into a joint. */
export function sliceCages(
  cages: readonly RebarCage[], cut: CageCut, thickness: number,
  keep?: (run: RebarRun) => boolean,
): CutTie[] {
  return cages.flatMap((c) => sliceCage(c, cut, thickness, keep))
}
