...
| Tension/compression-only members | `axialOnly.test.ts` | active-set iteration on a cross-braced bay: exactly one diagonal deactivates, the survivor is in tension (compression for comp-only struts), load reversal swaps WHICH diagonal is active, the braced bay is stiffer than the unbraced one, re-activation prevents latch-off, converges in ≤5 iterations; `meshValidation` rejects an axial release on a limited member and warns on nodes held only by limited members |
